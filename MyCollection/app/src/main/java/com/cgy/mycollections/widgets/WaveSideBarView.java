...
public class WaveSideBarView extends View {
    ...
    // 渲染字母表
    private List<String> mLetters = Arrays.asList("\u0023", "\u2606", "A", "B", "C", "D", "E", "F", "G", "H",
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mTextSize = dip2px(10);
        mLargeTextSize = dip2px(32);
        mPadding = dip2px(20);
        mRadius = dip2px(20);
        mBallRadius = dip2px(24);
...
//        }
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mCenterY = (int) y;
                startAnimator(true, mRatio, 1.0f);
    ...
    private void drawLetters(Canvas canvas) {
        ...
        canvas.drawRoundRect(rectF, mTextSize, mTextSize, mLettersPaint);//白色背景

        mLettersPaint.reset();
        mLettersPaint.setStyle(Paint.Style.STROKE);
        mLettersPaint.setColor(mTextColor);
        mLettersPaint.setAntiAlias(true);
        canvas.drawRoundRect(rectF, mTextSize, mTextSize, mLettersPaint);//边框线条
    ...
    /**
     * @param isShow 是显示动画还是隐藏动画
     * @param value
     */
    private void startAnimator(final boolean isShow, float... value) {
        if (mRatioAnimator == null) {
            mRatioAnimator = new ValueAnimator();
        }
        mRatioAnimator.cancel();
        mRatioAnimator.removeAllUpdateListeners();//因为addUpdateListener 里面是一个数组，此处需要移除所有的防止第二个 animator使用的时间和第一个的listener串了
        ...
        mRatioAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator value) {
                ...
                if (isShow && mRatio == 1f && oldChoose != newChoose) {
                    if (newChoose >= 0 && newChoose < mLetters.size()) {
                        mChoose = newChoose;
                        System.out.println("onLetterChange:" + mRatio + "---mChoose:" + mChoose);
                        if (listener != null) {
                            listener.onLetterChange(mLetters.get(newChoose));
                        }
                    }
