...
/**
 * Description : 通讯录 item decoration
 * Author :cgy
 * Date :2018/11/26
 */
public class ContactItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public ContactItemDecoration(Context context, List<ContactModel> data) {
        mData = data;
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0的时候绘制title
                    ...
                    if (null != mData.get(position).getNameFirstLetter() && !mData.get(position)
                            .getNameFirstLetter().equals(mData.get(position - 1).getNameFirstLetter())) {
    ...
    private void drawTitle(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        mPaint.getTextBounds(mData.get(position).getNameFirstLetter(), 0, mData.get(position).getNameFirstLetter().length(), mBounds);
        c.drawText(mData.get(position).getNameFirstLetter(),
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {
            ...
            if (position == 0) {
                ...
                if (null != mData.get(position).getNameFirstLetter() &&
                        !mData.get(position).getNameFirstLetter().equals(mData.get(position - 1).getNameFirstLetter())) {
