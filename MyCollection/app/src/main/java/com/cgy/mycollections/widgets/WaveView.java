...
public class WaveView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (iterator.hasNext()) {
            ...
            float outRadius = circle.getCurrentRadius();
            if (System.currentTimeMillis() - circle.mCreateTime < mDuration) {
                mPaint.setAlpha(circle.getAlpha());
                mPaint.setStrokeWidth(outRadius - mInitialRadius);

                float realRadius = (outRadius - mInitialRadius) / 2 + mInitialRadius;
                canvas.drawCircle(getWidth() / 2, getHeight() / 2, realRadius, mPaint);
//                RectF centerCircle=new RectF(getWidth() / 2-outRadius,getHeight() / 2-outRadius,getWidth() / 2+outRadius,getHeight() / 2+outRadius);
//                canvas.drawArc(centerCircle,0,360,false,mPaint);
            } else {
                iterator.remove();
            }
        }
    ...
    public void setOneWaveDuration(long duration) {
        mDuration = duration;
    }
